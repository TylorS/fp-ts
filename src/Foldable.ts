 ...
 * @since 3.0.0
 */
export interface Foldable1<F extends HKT> {
  readonly URI?: F
  readonly reduce: <B, A>(b: B, f: (b: B, a: A) => B) => (fa: Kind<F, A>) => B
  readonly foldMap: <M>(M: Monoid<M>) => <A>(f: (a: A) => M) => (fa: Kind<F, A>) => M
  readonly reduceRight: <B, A>(b: B, f: (a: A, b: B) => B) => (fa: Kind<F, A>) => B
 ...
 * @since 3.0.0
 */
export interface Foldable2<F extends HKT2> {
  readonly URI?: F
 ...
 * @since 3.0.0
 */
export interface Foldable2C<F extends HKT2, E> {
  readonly URI?: F
  readonly _E?: E
 ...
 * @since 3.0.0
 */
export interface Foldable3<F extends HKT3> {
  readonly URI?: F
 ...
 * @since 3.0.0
 */
export interface Foldable3C<F extends HKT3, E> {
  readonly URI?: F
  readonly _E?: E
 ...
 * @since 3.0.0
 */
export interface Foldable4<F extends HKT4> {
  readonly URI?: F
...
export function reduceE<F extends HKT>(
  ...
  <M extends HKT4>(M: Chain4<M>): <S, R, E, B, A>(
    mb: Kind4<M, S, R, E, B>,
    f: (b: B, a: A) => Kind4<M, S, R, E, B>
  ) => (fa: Kind<F, A>) => Kind4<M, S, R, E, B>
  <M extends HKT3>(M: Chain3<M>): <R, E, B, A>(
    mb: Kind3<M, R, E, B>,
    f: (b: B, a: A) => Kind3<M, R, E, B>
  ) => (fa: Kind<F, A>) => Kind3<M, R, E, B>
  <M extends HKT3, E>(M: Chain3C<M, E>): <R, B, A>(
    mb: Kind3<M, R, E, B>,
    f: (b: B, a: A) => Kind3<M, R, E, B>
  ) => (fa: Kind<F, A>) => Kind3<M, R, E, B>
  <M extends HKT2>(M: Chain2<M>): <E, B, A>(
    mb: Kind2<M, E, B>,
    f: (b: B, a: A) => Kind2<M, E, B>
  ) => (fa: Kind<F, A>) => Kind2<M, E, B>
  <M extends HKT2, E>(M: Chain2C<M, E>): <B, A>(
    mb: Kind2<M, E, B>,
    f: (b: B, a: A) => Kind2<M, E, B>
  ) => (fa: Kind<F, A>) => Kind2<M, E, B>
  <M extends HKT>(M: Chain1<M>): <B, A>(
    mb: Kind<M, B>,
    f: (b: B, a: A) => Kind<M, B>
...
export function reduceE<F>(
  F: Foldable<F>
): <M>(M: Chain<M>) => <B, A>(mb: Kind<M, B>, f: (b: B, a: A) => Kind<M, B>) => (fa: Kind<F, A>) => Kind<M, B>
export function reduceE<F>(
  F: Foldable<F>
): <M>(M: Chain<M>) => <B, A>(mb: Kind<M, B>, f: (b: B, a: A) => Kind<M, B>) => (fa: Kind<F, A>) => Kind<M, B> {
  return (M) => (mb, f) =>
 ...
 * @since 3.0.0
 */
export function intercalate<F extends HKT4>(
  F: Foldable4<F>
): <M>(M: Monoid<M>) => (sep: M) => <S, R, E>(fm: Kind4<F, S, R, E, M>) => M
export function intercalate<F extends HKT3>(
  F: Foldable3<F>
): <M>(M: Monoid<M>) => (sep: M) => <R, E>(fm: Kind3<F, R, E, M>) => M
export function intercalate<F extends HKT3, E>(
  F: Foldable3C<F, E>
): <M>(M: Monoid<M>) => (sep: M) => <R>(fm: Kind3<F, R, E, M>) => M
export function intercalate<F extends HKT2>(
  F: Foldable2<F>
): <M>(M: Monoid<M>) => (sep: M) => <E>(fm: Kind2<F, E, M>) => M
export function intercalate<F extends HKT2, E>(
  F: Foldable2C<F, E>
): <M>(M: Monoid<M>) => (sep: M) => (fm: Kind2<F, E, M>) => M
export function intercalate<F extends HKT>(F: Foldable1<F>): <M>(M: Monoid<M>) => (sep: M) => (fm: Kind<F, M>) => M
export function intercalate<F>(F: Foldable<F>): <M>(M: Monoid<M>) => (sep: M) => (fm: Kind<F, M>) => M
export function intercalate<F>(F: Foldable<F>): <M>(M: Monoid<M>) => (sep: M) => (fm: Kind<F, M>) => M {
  return <M>(M: Monoid<M>) => (sep: M) => (fm: Kind<F, M>) => {
 ...
 * @since 3.0.0
 */
export function toReadonlyArray<F extends HKT4>(
  F: Foldable4<F>
): <S, R, E, A>(fa: Kind4<F, S, R, E, A>) => ReadonlyArray<A>
export function toReadonlyArray<F extends HKT3>(F: Foldable3<F>): <R, E, A>(fa: Kind3<F, R, E, A>) => ReadonlyArray<A>
export function toReadonlyArray<F extends HKT3, E>(
  F: Foldable3C<F, E>
): <R, A>(fa: Kind3<F, R, E, A>) => ReadonlyArray<A>
export function toReadonlyArray<F extends HKT2>(F: Foldable2<F>): <E, A>(fa: Kind2<F, E, A>) => ReadonlyArray<A>
export function toReadonlyArray<F extends HKT2, E>(F: Foldable2C<F, E>): <A>(fa: Kind2<F, E, A>) => ReadonlyArray<A>
export function toReadonlyArray<F extends HKT>(F: Foldable1<F>): <A>(fa: Kind<F, A>) => ReadonlyArray<A>
export function toReadonlyArray<F>(F: Foldable<F>): <A>(fa: Kind<F, A>) => ReadonlyArray<A>
export function toReadonlyArray<F>(F: Foldable<F>): <A>(fa: Kind<F, A>) => ReadonlyArray<A> {
  return <A>(fa: Kind<F, A>) =>
    pipe(
      fa,
